...
type AuthzService interface {
	...
	CheckAccess(token, permission, resource string) (access int, err error)
}
...
//
func AuthzFromRuntime(config *RuntimeConfig) AuthzService {
	if nil == fenceSingleton {
		fenceSingleton = FenceConfigFromRuntime(config).Build()
	}
